<script>
    ...
    let isInjectorSet = $derived(
        currentType ? isImplementing(currentType, "ResourceInjector") : false,
    );
    let injecting = $state(isInjectorSet);
    let canInject = $derived(
        isInjectionAllowed(
            parameterSpec,
            context.isFactory,
            context.isWithinFactory,
        ),
    );
    ...
    let descriptionHtml = $derived(
        parameterSpec?.description
            ? marked.parseInline(parameterSpec.description)
            : "",
    );
    let showTooltip = $state(false);

    let isRepresentingMnemonic = $derived(
        !injecting &&
            !isPrim &&
...
{#snippet fieldContent()}
    {#if isMapInjection(parameterSpec)}
        <MapValue yaml={parameterYaml} {parameterSpec} {context} {onchange} />
    ...
    {:else}
        <div class="direct-field">
            ...
            {:else if isPrim}
                ...
                {:else}
                    {console.error(
                        `ParameterField: unhandled primitive category for mnemonic "${mnemonic}"`,
                    )}
                {/if}
            {:else}
                <MnemonicValue
                    yaml={parameterYaml}
                    mnemonic={parameterSpec.mnemonic}
                    {context}
                    {onchange}
                />
            {/if}
